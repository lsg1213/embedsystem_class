...
#define DEVICE_NAME			"sjl_fpga_textlcd"
...
static int sjl_fpga_textlcd_open(struct inode * inode, struct file * file)
{
	if (textlcd_usage == -1)
		return -EBUSY;
...
static int sjl_fpga_textlcd_release(struct inode * inode, struct file * file)
{
...
/* not use
static ssize_t sjl_fpga_textlcd_read(struct file * file, char * buf, size_t length, loff_t * ofs)
{
	printk("sjl_fpga_textlcd_read, \n");
	
	return 0;
}
*/

static ssize_t sjl_fpga_textlcd_write(struct file * file, const char * buf, size_t length, loff_t * ofs)
{
	int i;
	int size;
...
static DEFINE_MUTEX(sjl_fpga_textlcd_mutex);
static long sjl_fpga_textlcd_ioctl(struct file * file, unsigned int cmd, unsigned long arg)
{
	...
	default:
		mutex_unlock(&sjl_fpga_textlcd_mutex);
		return ENOTTY;
	}
...
static struct file_operations sjl_fpga_textlcd_fops = {
	.owner = THIS_MODULE,
	.open = sjl_fpga_textlcd_open,
	.release = sjl_fpga_textlcd_release,
//	.read = sjl_fpga_textlcd_read,
	.write = sjl_fpga_textlcd_write,
	.unlocked_ioctl = sjl_fpga_textlcd_ioctl,
};

static struct miscdevice sjl_fpga_textlcd_driver = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "sjl_fpga_textlcd",
	.fops = &sjl_fpga_textlcd_fops,
};

static int sjl_fpga_textlcd_init(void){
	printk("sjl_fpga_textlcd_init, \n");
	
	return misc_register(&sjl_fpga_textlcd_driver);
}

static void sjl_fpga_textlcd_exit(void){
	printk("sjl_fpga_textlcd_exit, \n");

	misc_deregister(&sjl_fpga_textlcd_driver);
	
}

module_init(sjl_fpga_textlcd_init);
module_exit(sjl_fpga_textlcd_exit);

MODULE_AUTHOR("Hanback");
MODULE_DESCRIPTION("sjl_fpga_textlcd driver test");
